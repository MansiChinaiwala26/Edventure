<%-- 
    Document   : edit
    Created on : Oct 25, 2015, 5:01:08 PM
    Author     : user
--%>
        ...
        <table cellpadding="5">
            ...
            <%
                ...
                try {
                    ...
                    Connection cn = DriverManager.getConnection("jdbc:mysql://localhost:3306/edventure", "root", "");
                    Statement stmt = null;
                    PreparedStatement st = null;
                    String ID = null;
                    String q = "SELECT * FROM jobdetail";
                    ...
                    String query = "UPDATE jobdetail SET Name=?,Age=?,Address=?,State=?,City=?,Email=?,ContactNo=?,Designation=?,BranchofStudy=?,Company=?,Degree=?,ExpectedSalary=?,Experience=? WHERE ID=?";
                    st = cn.prepareStatement(query);
                    st.setString(1, request.getParameter("name"));
                    st.setString(2, request.getParameter("age"));
                    st.setString(3, request.getParameter("add"));
                    st.setString(4, request.getParameter("sname"));
                    st.setString(5, request.getParameter("cname"));
                    st.setString(6, request.getParameter("ename"));
                    st.setString(7, request.getParameter("ctname"));
                    st.setString(8, request.getParameter("desname"));
                    st.setString(9, request.getParameter("branch"));
                    st.setString(10, request.getParameter("cyname"));
                    st.setString(11, request.getParameter("degree"));
                    st.setString(12, request.getParameter("exps"));
                    st.setString(13, request.getParameter("expyear"));
                    st.setString(14, ID);


                    //String query = "insert into student(First_Name,Last_Name,Gender,Date_Of_Birth,Address,City,State,Email,Contact_no,Qualification,User_id,Password) values ('"+fname+"','"+lname+"','"+gender+"','"+bdate+"','"+add+"','"+cname+"','"+sname+"','"+ename+"',"+ctname+",'"+qname+"','"+uname+"','"+pname+"')";

                    st.executeUpdate();
                    String to = request.getParameter("ename");
               
                String msg = "Your EDventure Account have been Successfully Updated ";
            ...
            %>
            <td height="24" colspan="4"><div align="center" class="style4"><font size="12">Account Updated Successfully</font></div></td>
            <tr><td></td></tr>
            <tr><td></td></tr>
        </table>
    </center>
